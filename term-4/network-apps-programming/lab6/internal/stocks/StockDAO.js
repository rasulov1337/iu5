...
class StockDAO {
    constructor(id, src, title, text, visited_times=0) {
        this.id = id;
        this.src = src;
        this.title = title;
        this.text = text;
    ...
    static find() {
        ...
        return stocks.map(({id, src, title, text, visited_times}) => {
            return new this(id, src, title, text, visited_times);
        });
    }
    ...
    static findById(id) {
        ...
        return new this(stock.id, stock.src, stock.title, stock.text, stock.visited_times);
    }
    ...
    static delete(id) {
        ...
        return filteredStocks.map(({id, src, title, text, visited_times}) => {
            return new this(id, src, title, text, visited_times);
        });
    }
    ...
    toJSON() {
        return {
            ...
            title: this.title,
            text: this.text,
            visited_times: this.visited_times
        }
    }
}
